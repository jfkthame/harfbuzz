...
struct output_buffer_t
{
  ...
  void dump_image (GString *image_data, int image_num)
  {
    if (!options.output_file)
      return;
    GString *base = g_string_new (options.output_file);
    if (!strcmp (base->str + base->len - 6, ".diffs"))
      g_string_truncate (base, base->len - 6);
    GString *dir = g_string_new (0);
    g_string_printf (dir, "%s.images/%d", base->str, line_no / 1000);
    g_string_free (base, true);
    if (0 != g_mkdir_with_parents (dir->str, 0755)) {
      g_string_free (dir, true);
      return;
    }
    GString *name = g_string_new (0);
    g_string_printf (name, "%s/%d.image%d.png", dir->str, line_no, image_num);
    g_string_free (dir, true);
